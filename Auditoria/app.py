...
class Company(db.Model):
    ...
    legal_rep = db.Column(db.String(120))
    website = db.Column(db.String(120))
    sector = db.Column(db.String(120))
    type_company = db.Column(db.String(120))
    address = db.Column(db.String(200))
    social_links = db.Column(db.String(250))
...
def register():
    if request.method == 'POST':
        u = User(username=request.form['username'], email=request.form['email'], role=request.form.get('role','user'))
...
def company():
    if request.method == 'POST':
        c = Company(
            ...
            legal_rep=request.form['legal_rep'],
...
def documents():
    ...
    # Obtener documentos y usuarios que los subieron
...
def delete_document(doc_id):
    d = Document.query.get_or_404(doc_id)
    # Solo el usuario que subió el documento o un admin puede eliminar
...
#-------cd /workspaces/Audioria-/Auditoria python3 -m venv .venv source .venv/bin/activate-------
